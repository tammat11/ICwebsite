...
const Hero = ({ onCalcOpen }: { onCalcOpen?: () => void }) => {
    ...
    return (
        <section ref={root} className="relative h-screen flex flex-col items-center justify-center px-6 overflow-hidden bg-white">
            ...
            <div className="max-w-7xl mx-auto text-center relative z-10 w-full flex flex-col items-center">
                ...
                <h1 className="flex flex-col items-center mb-6 select-none font-bold tracking-tighter uppercase whitespace-nowrap leading-[0.8] relative z-20">
                    <span className="hero-title-reveal block text-[clamp(50px,13vw,100px)] text-brand-dark">
                        Создать
                    </span>
                    <span className="hero-title-reveal block text-[clamp(55px,14vw,120px)] text-brand-green relative inline-block px-4 py-2 overflow-visible group">
                        <span className="relative z-20">Чистоту</span>
                ...
                <div className="hero-fade-in max-w-4xl mb-8 px-6 text-center z-20">
                    <p className="text-base md:text-xl font-bold text-brand-dark/50 tracking-tight leading-relaxed max-w-2xl drop-shadow-sm">
                        <span className="text-brand-dark">Профессиональный клининг.</span> <br className="hidden md:block" />
                        IC Group — самая крупная компания, предоставляющая <br className="hidden md:block" />
                        профессиональные клининговые услуги на рынке Казахстана.
                    </p>
                </div>

                <div className="hero-fade-in flex flex-col items-center mt-2 z-20">
                    <SparklesButton onClick={onCalcOpen} className="px-12 py-6 text-xs bg-brand-dark hover:bg-brand-green shadow-xl">
