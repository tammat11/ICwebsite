    ...
    return (
        <section ref={sectionRef} className={`py-20 bg-white border-y border-black/5 overflow-hidden ${inView ? 'in-view' : ''}`}>
            ...
            <div className="relative">
                    ...
                    {[1, 2].map((set) => (
                        <div key={set} className="flex items-center gap-12 md:gap-24">
                            {clients.map((client, i) => (
                                ...
                                >
                                    <div className="w-16 h-16 md:w-32 md:h-32 rounded-full bg-white border border-black/5 flex items-center justify-center p-4 md:p-6 transition-all duration-500 group-hover:border-brand-green/30 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.05)] group-hover:-translate-y-2 relative overflow-hidden">
                                        <img
                ...
                <div className="absolute inset-y-0 left-0 w-24 md:w-48 bg-gradient-to-r from-white via-white/80 to-transparent z-20" />
                <div className="absolute inset-y-0 right-0 w-24 md:w-48 bg-gradient-to-l from-white via-white/80 to-transparent z-20" />
            </div>
        </section>
    );
};
