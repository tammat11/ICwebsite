            ...
            <main className="max-w-7xl mx-auto px-6 pt-32 pb-32">
                <div className="grid lg:grid-cols-[1fr_1.1fr] gap-20 items-start">
                    ...
                    <div className="sticky top-32">
                        <button 
                            onClick={() => navigate(-1)}
                            className="inline-flex items-center gap-2 text-brand-dark/30 hover:text-brand-green transition-all font-black uppercase tracking-[0.3em] text-[10px] mb-16 group"
                        >
                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <h1 className="text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter text-brand-dark uppercase leading-[0.85] mb-8">
                                    Станьте <span className="text-brand-green">частью</span> команды
                                </h1>
                                <p className="text-xl md:text-2xl text-brand-dark/40 font-medium leading-relaxed max-w-xl">
                            ...
                            <div className="grid gap-6">
                                {[
                                    { title: 'HR воронка', text: 'Заявка мгновенно попадает к рекрутерам' },
                                    { title: 'Прямая связь', text: 'Никаких посредников и долгих ожиданий' },
                                    { title: 'Прозрачность', text: 'Все этапы отбора в реальном времени' }
                                ].map((item, i) => (
                                    <div key={i} className="flex gap-4 items-start p-6 rounded-3xl bg-white border border-black/[0.03] shadow-sm">
                                        <div className="w-1.5 h-1.5 rounded-full bg-brand-green mt-2.5 flex-shrink-0" />
                                        <div>
                                            <h3 className="font-black text-xs uppercase tracking-widest text-brand-dark mb-1">{item.title}</h3>
                                            <p className="text-sm text-brand-dark/40 font-medium">{item.text}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="relative">
                        ...
                        <div className="bg-white rounded-[56px] p-8 md:p-14 shadow-[0_40px_100px_rgba(0,0,0,0.04)] border border-black/[0.03] relative z-10">
                            <form onSubmit={handleSubmit} className="space-y-10">
                                ...
                                <button
                                    ...
                                    className={`w-full bg-brand-dark text-white py-7 rounded-[28px] font-black uppercase tracking-[0.3em] text-xs hover:bg-brand-green transition-all shadow-[0_20px_60px_rgba(0,0,0,0.15)] hover:shadow-[0_20px_60px_rgba(162,227,43,0.3)] hover:-translate-y-1 active:translate-y-0 group flex items-center justify-center gap-4 ${isSubmitting ? 'opacity-80' : ''}`}
                                >
                                    {isSubmitting ? (
